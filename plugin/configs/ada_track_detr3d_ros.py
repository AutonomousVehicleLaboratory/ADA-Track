...
test_pipeline = [
    dict(type='LoadMultiViewImageFromFilesROS'),
    dict(type='Normalize3D', **img_norm_cfg),
    dict(type='Pad3D', size_divisor=32),
]
...
test_pipeline_post = [
    ...
    dict(type='Collect3D', keys=['img', 'timestamp', 'l2g_r_mat', 'l2g_t'],
         meta_keys=('filename', 'ori_shape', 'img_shape', 'lidar2img',
                    ...
                    'pcd_scale_factor', 'pcd_rotation', 'pts_filename',
                    'transformation_3d_flow', 'scene_token'))]
# needed: img_shape, box_type_3d, box_mode_3d(Box3DMode.CAM), lidar2img: trasformation between lidar to cameras (list of 4x4 array)
# scene_token
data = dict(
